...
const AccountSlice = createSlice({
    ...
    reducers: {
        ...
        saveUserAccounts: (state, action: PayloadAction<AccountResponse[] | []>) => {
            state.userAccounts = action.payload
        },
        saveAccountStats: (state, action: PayloadAction<AccountResponse[] | []>) => {
            const balance = {
                savings: 0,
                current: 0,
                creditCard: 0,
...
export const { changeLoading, saveUserAccounts, saveAccountStats } = AccountSlice.actions;

export const fetchUserAccounts = (userId: string): AppThunk<Promise<void>> => async (dispatch) => {
